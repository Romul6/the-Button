...
export class BookingModal implements OnInit {
    ...
    public opponent = new FormControl<string | null>(null);
    ...
    ngOnInit(): void {
        this.filteredOpponenets = this.opponent.valueChanges.pipe(
            startWith(''),
            map(value => this._filter(value || '')),
        );
    }

    private _filter(value: string): idname[] {
        const filterValue = value.toLowerCase();
        return this.opponents.filter(option => option.name.toLowerCase().includes(filterValue));
    }
